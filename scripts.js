document.addEventListener("DOMContentLoaded", () => {
	...
	function handleSubmitWord() {
		const currentWordArr = getCurrentWordArr();
		if (currentWordArr.length !== 5 && removeUserInteract != 1) {
			...
			if (currentWord === word) {
				removeUserInteract = 1;
				setTimeout(() => {window.alert("correct")}, 2100);
				
			}

			else if (guessedWords.length === 6) {
				removeUserInteract = 1;
				setTimeout(() => {window.alert("Out of attempts. Word is " + word + ".")}, 2100); 
			}
	...
	function handleDeleteLetter() {
		if (deletable === 0 && removeUserInteract != 1) {
			const currentWordArr = getCurrentWordArr();
	...
	//takes keyboard press and sends it to the updater
	function logKey(event) {
		if(removeUserInteract != 1) {
		if (event.key === "Enter") {
			handleSubmitWord();
			return;
		} 
			...
			//else if (event.key.charCodeAt(0) > 96 && event.key.charCodeAt(0) < 123) 
			else if(event.code.substring(0,3) == "Key") {
			updateGuessedWords(String(event.key));
			deletable = 0;
		}
		}
	}
